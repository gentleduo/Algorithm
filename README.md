...
根据等差数列求和可知整个流程需要Sn=na1+n(n-1)d/2次比较常数操作（a1是首项），和n次交换常数操作，将两部分加起来得到整个流程的所有常数操作和为：a*N^2+b*N+c（a、b、c为常数，也可以理解为a，b，c为固定时间的常数操作的耗时），复杂度则只看表达式最高阶项的部分，低阶项以及高阶项前面的系数都不要，所以这个算法的时间复杂度为：O(N^2)。复杂度是确定算法流程的总操作数量(常数操作)与样本数量之间的表达式关系。
...
public class HeapDef {
    ...
    public static class MyMaxHeap {
        ...
        // 构造小根堆只要改变新加进来的数的判断逻辑即可：index位置的数小于(index - 1)/2的数则往上移
        ...
        // 构造小根堆只要改变新加进来的数的判断逻辑即可：较小的孩子都不再比index位置的数小了，终止
...
稳定性的意义：如果⼀组数据只需要⼀次排序，则稳定性⼀般是没有意义的，如果⼀组数据需要多次排序，稳定性是有意义的。例如要排序的内容是⼀组商品对象，第⼀次排序按照价格由低到⾼排序，第⼆次排序按照销量由⾼到低排序，如果第⼆次排序使⽤稳定性算法，就可以使得相同销量的对象依旧保持着价格⾼低的顺序展现，只有销量不同的对象才需要重新排序。这样既可以保持第⼀次排序的原有意义，⽽且可以减少系统开销。
...
第一次按价格排完后：三星Note3、华为p3、华为mate2、Iphone15

第二次按销量排完后：华为p3、华为mate2、三星Note3、Iphone15

因为二次排序是稳定的，所以当销量一致时，不会改变其相对顺序（即：不会影响销量相同的商品在第一次排序后的相对次序），而由于第一次排序使得这组商品对象已经按价格由低到高排列了，所以第二次排序后销量相同的商品，价格一定是由低到高排列的。
