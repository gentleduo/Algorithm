package org.duo.master.chapter003;
...
/**
 * 哈希表的增、删、查、改时间复杂度都是O(1)，只是这个常数操作的耗时会有点长
 * 当哈希表中的K,V是基本类型时，哈希表存储的是该基础类型的实际值，而当K,V是非基本类型的时候，哈希表存储的是对象的引用
 *
 * 有序表，比如 TreeMap；增删改查的时间复杂度为O(logN)
 *
 */
